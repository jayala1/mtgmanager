...
class CardBrowser:
    ...
    def display_card_details(self, card_data: Dict[str, Any]):
        ...
        # FIXED: Use card_data instead of card
    ...
    def add_selected_to_inventory(self, event=None):
        ...
        dialog.geometry("350x300") # Made taller and wider
        ...
        # Card info
        info_frame = ttk.LabelFrame(main_container, text="Card to Add")
        info_frame.pack(fill=tk.X, pady=(0, 15))
        
        card_name = self.current_card_data.get('name', 'Unknown Card')
        ttk.Label(info_frame, text=card_name, font=('TkDefaultFont', 11, 'bold')).pack(padx=10, pady=10)
        ...
        quantity_spin = ttk.Spinbox(quantity_frame, from_=1, to=100, textvariable=quantity_var, width=8)
        quantity_spin.pack(side=tk.LEFT, padx=5)
        
        # Foil
        foil_frame = ttk.Frame(options_frame)
        foil_frame.pack(fill=tk.X, padx=10, pady=5)
        
        foil_var = tk.BooleanVar()
        ttk.Checkbutton(foil_frame, text="Foil", variable=foil_var).pack(side=tk.LEFT)
        
        # Condition
        condition_frame = ttk.Frame(options_frame)
        condition_frame.pack(fill=tk.X, padx=10, pady=(5, 10))
        
        ttk.Label(condition_frame, text="Condition:", width=10).pack(side=tk.LEFT)
        ...
        add_button = ttk.Button(button_frame, text="Add", command=add_card)
